...
(function (OC, FileInfo) {
   ...
   */
  [Client.NS_OWNCLOUD, 'permissions'], // [Client.NS_OWNCLOUD, 'downloadURL'],
  ...
  Client.prototype = {
    ...
    _xhrProvider: function () {
      ...
      const oldOpen = xhr.open; // override open() method to add headers
    ...
    _simpleCall: function (method, path) {
      ...
      this._client.request(method, this._buildUrl(path)).then(function (result) {
        if (self._isSuccessStatus(result.status)) {
          deferred.resolve(result.status);
        } else {
          result = _.extend(result, self._getSabreException(result));
...
/******/ })()
;
//# sourceMappingURL=core-files_client.js.map?v=e643ebeb049f125b7d6f